...
<H2> Prenotazione Ristorante PROVAMI</H2>
<BR>

	<?php 
		if (isset($_SESSION['session_id'])) {
		// htmlspecialchars - Funzione per evitare inizione di codice dannoso
		$session_user = htmlspecialchars($_SESSION['session_user'], ENT_QUOTES, 'UTF-8');
		$session_id = htmlspecialchars($_SESSION['session_id']);
			
			printf("Benvenuto <b>%s</b> , la tua session ID è <b>%s</b>", $session_user, $session_id);
			echo "<br>";
...
<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"> 
  Utente registrato: <?php echo $session_user; ?>
...
<?php
 ...
 {
	 echo "<BR><BR><hr>";
	 echo "Grazie <b>".$session_user."</b><br><br>";
